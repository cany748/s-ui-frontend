  ...
  <v-card style="background-color: inherit">
    <v-row>
      ...
      <v-col cols="12" sm="6" md="4" v-if="optionNetwork">
        <v-select hide-details multiple chips :label="$t('network')" :items="['tcp', 'udp', 'icmp']" v-model="rule.network"> </v-select>
      </v-col>
    ...
    <v-row v-if="optionInterface">
      <v-col cols="12" sm="6" md="4">
        <v-select hide-details :items="interfaceKeys" @update:model-value="updateInterfaceOption($event)" v-model="interfaceOption">
        </v-select>
      </v-col>
      <v-col
        cols="12"
  ...
  computed: {
    ...
    source_port: {
      ...
      set(v: string) {
        ...
        if (!v.endsWith("\n")) {
          this.$props.rule.source_port =
            lines.length > 0 ? lines.map((str: string) => parseInt(str, 10)).filter((n: number) => !isNaN(n)) : [];
        }
      },
    },
    source_port_range: {
