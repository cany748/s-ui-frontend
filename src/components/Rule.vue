<template>
  ...
  <v-card style="background-color: inherit">
    <v-row>
      ...
      <v-col v-if="optionNetwork" cols="12" sm="6" md="4">
        <v-select v-model="rule.network" hide-details multiple chips :label="$t('network')" :items="['tcp', 'udp', 'icmp']"> </v-select>
      </v-col>
      <v-col v-if="optionProtocol" cols="12" sm="6">
        <v-select v-model="rule.protocol" :items="protocols" :label="$t('protocol')" multiple chips hide-details></v-select>
      </v-col>
    </v-row>
    <v-row v-if="optionDomain">
      ...
      <v-col v-if="rule.ip_cidr != undefined" cols="12" sm="6">
        <v-textarea
          v-model="ip_cidr"
          :label="$t('rule.ip')"
          hide-details
      ...
      <v-col v-if="rule.ip_is_private != undefined" cols="12" sm="6">
    ...
    <v-row v-if="optionSrcIP">
      ...
      <v-col v-if="rule.source_ip_cidr != undefined" cols="12" sm="6">
        <v-textarea
      ...
      <v-col v-if="rule.source_ip_is_private != undefined" cols="12" sm="6">
    ...
    <v-row v-if="optionInterface">
      <v-col cols="12" sm="6" md="4">
        <v-select v-model="interfaceOption" hide-details :items="interfaceKeys" @update:model-value="updateInterfaceOption($event)">
        </v-select>
      </v-col>
      <v-col
        v-if="
          rule.interface_address != undefined || rule.network_interface_address != undefined || rule.default_interface_address != undefined
        "
        cols="12"
        sm="6"
      >
    ...
    <v-row v-if="optionRuleSet">
      <v-col cols="12" sm="6">
        <v-combobox v-model="rule.rule_set" :items="rsTags" :label="$t('rule.ruleset')" multiple chips hide-details></v-combobox>
      </v-col>
      <v-col cols="12" sm="6">
...
export default {
  ...
  computed: {
    ...
    optionPreferredBy: {
      get() {
        return this.$props.rule.preferred_by != undefined;
      },
      set(v: boolean) {
        this.$props.rule.preferred_by = v ? [] : undefined;
      },
    },
    optionInterface: {
      get() {
        return this.interfaceKeys.some((k) => this.$props.rule[k] != undefined);
      },
      set(v: boolean) {
        if (v) {
          this.$props.rule.interface_address = [];
        } else {
          for (const k of this.interfaceKeys) delete this.$props.rule[k];
        }
    ...
    optionNetwork: {
      get() {
        return this.$props.rule.network != undefined;
      },
      set(v: boolean) {
        this.$props.rule.network = v ? [] : undefined;
      },
    },
    domain: {
      get() {
    ...
    domain_keyword: {
      ...
      set(v: string) {
        this.$props.rule.domain_keyword =
          v.length > 0
            ? v
                .split("\n")
                .map((s: string) => s.trim())
    ...
    port: {
      ...
      set(v: string) {
        ...
        if (!v.endsWith("\n")) {
          this.$props.rule.port =
            lines.length > 0 ? lines.map((str: string) => Number.parseInt(str, 10)).filter((n: number) => !isNaN(n)) : [];
        }
      },
    },
    port_range: {
    ...
    source_port_range: {
      ...
      set(v: string) {
        this.$props.rule.source_port_range =
          v.length > 0
            ? v
                .split("\n")
                .map((s: string) => s.trim())
