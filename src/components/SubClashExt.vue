  ...
  <v-card>
    ...
    <v-row v-if="optionRules">
      <v-col cols="12" sm="12" md="6" lg="4">
        <v-select v-model="rules" :items="rulesIP" chips closable-chips multiple hide-details :label="$t('pages.rules')"></v-select>
      </v-col>
    </v-row>
    <v-card-actions>
      <v-spacer></v-spacer>
...
export default {
    ...
    return {
      ...
      defaultConfig: {
        ...
        dns: {
          ...
          nameserver: ["https://doh.pub/dns-query", "https://1.0.0.1/dns-query"],
  ...
  computed: {
    ...
    optionMixed: {
      get() {
        return this.metaJson["mixed-port"] > 0;
      },
      set(v: boolean) {
    ...
    optionTun: {
      get() {
        return this.metaJson["tun"]?.["enable"] ?? false;
      },
      set(v: boolean) {
        this.updateMetaJson(v ? this.defaultConfig["tun"] : null, "tun");
      },
    },
    optionExt: {
      get() {
        return this.metaJson["external-controller"]?.length > 0;
      },
      set(v: boolean) {
        this.updateMetaJson(v ? this.defaultConfig["external-controller"] : null, "external-controller");
      },
    },
    optionLog: {
      get() {
        return this.metaJson["log-level"]?.length > 0;
      },
      set(v: boolean) {
        this.updateMetaJson(v ? this.defaultConfig["log-level"] : null, "log-level");
      },
    },
    optionDns: {
      get() {
        return this.metaJson["dns"]?.["enable"] ?? false;
      },
      set(v: boolean) {
        this.updateMetaJson(v ? this.defaultConfig["dns"] : null, "dns");
      },
    },
    optionRules: {
      get() {
        return this.metaJson["rules"]?.length > 0;
      },
      set(v: boolean) {
    ...
    mixedPort: {
      get() {
        return this.metaJson["mixed-port"];
      },
      set(v: number) {
        this.updateMetaJson(v, "mixed-port");
      },
    },
    allowLan: {
      get() {
        return this.metaJson["allow-lan"];
      },
      set(v: boolean) {
        this.updateMetaJson(v, "allow-lan");
      },
    },
    externalController: {
      get() {
        return this.metaJson["external-controller"];
      },
      set(v: string) {
        this.updateMetaJson(v, "external-controller");
      },
    },
    logLevel: {
      get() {
        return this.metaJson["log-level"];
      },
      set(v: string) {
        this.updateMetaJson(v, "log-level");
      },
    },
    dns: {
      get() {
      ...
      set(v: boolean) {
        this.updateMetaJson({ ...this.metaJson["dns"], enable: v }, "dns");
      },
    },
    tun: {
      get() {
      ...
      set(v: boolean) {
        this.updateMetaJson({ ...this.metaJson["tun"], enable: v }, "tun");
      },
    },
    rules: {
      get() {
        return this.metaJson.rules.length > 0 ? this.metaJson.rules.filter((r: string) => r != "MATCH,Proxy") : [];
      },
      set(v: string[]) {
        let newRules = <string[]>[];
        v.forEach((r: string) => {
