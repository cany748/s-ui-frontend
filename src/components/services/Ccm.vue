<template>
  <v-card subtitle="CCM (Claude Code Multiplexer)">
    <v-row>
      <v-col cols="12" sm="6">
        <v-text-field v-model="data.credential_path" :label="$t('types.ccm.credentialPath')" hide-details> </v-text-field>
      </v-col>
      <v-col cols="12" sm="6">
        <v-text-field v-model="data.usages_path" :label="$t('types.ccm.usagesPath')" hide-details> </v-text-field>
      </v-col>
      <v-col cols="12" sm="6">
    ...
    <v-card v-for="(user, index) in data.users || []" :key="index" class="border" style="margin: 4px; padding: 8px" rounded="xl">
      <v-row>
        ...
        <v-col cols="12" sm="4">
          <v-text-field v-model="user.name" :label="$t('types.ccm.userName')" hide-details />
        </v-col>
        <v-col cols="12" sm="6">
          <v-text-field v-model="user.token" :label="$t('types.ccm.userToken')" hide-details type="password" />
        </v-col>
      </v-row>
    </v-card>
  </v-card>
