      ...
      <v-card-title>
        <v-row>
          ...
          <v-col cols="auto" align-self="center" justify-self="center">
            <v-chip color="primary" density="compact" variant="elevated" @click="data.verify_client_url.push({ url: '' })"
    ...
    <template v-if="optionMesh">
      <v-card-title>
        <v-row>
          ...
          <v-col cols="auto" align-self="center" justify-self="center">
            <v-chip color="primary" density="compact" variant="elevated" @click="data.mesh_with.push({ tls: {} })"
              ><v-icon icon="mdi-plus"
            /></v-chip>
          </v-col>
        </v-row>
      ...
      <v-row>
        <v-col cols="auto">
          <v-btn-toggle v-model="usePskText" class="rounded-xl" density="compact" variant="outlined" shaped mandatory>
            <v-btn @click="delete data.mesh_psk_file">{{ $t("types.derp.meshPsk") }}</v-btn>
            <v-btn @click="delete data.mesh_psk">{{ $t("types.derp.meshPskFile") }}</v-btn>
          </v-btn-toggle>
        </v-col>
      </v-row>
...
export default {
  ...
  computed: {
    optionVerifyCE: {
      get() {
        return this.$props.data.verify_client_endpoint != undefined;
      },
      set(v: boolean) {
        this.$props.data.verify_client_endpoint = v ? [] : undefined;
      },
    },
    optionVerifyCU: {
      get() {
        return this.$props.data.verify_client_url != undefined;
      },
      set(v: boolean) {
        this.$props.data.verify_client_url = v ? [{ url: "" }] : undefined;
      },
    },
    optionHome: {
      get() {
        return this.$props.data.home != undefined;
      },
      set(v: boolean) {
        this.$props.data.home = v ? "" : undefined;
      },
    },
    optionMesh: {
      get() {
        return this.$props.data.mesh_with != undefined;
      },
      set(v: boolean) {
        if (v) {
    ...
    optionStun: {
      get() {
        return this.$props.data.stun != undefined;
      },
      set(v: boolean) {
        this.$props.data.stun = v ? { enabled: true } : undefined;
      },
    },
  },
