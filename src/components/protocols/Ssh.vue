...
export default {
  ...
  computed: {
    ...
    optionHostKey: {
      get(): boolean {
        return this.data.host_key != undefined;
      },
      set(v: boolean) {
        this.data.host_key = v ? "" : undefined;
      },
    },
    optionAlgorithms: {
      get(): boolean {
        return this.data.host_key_algorithms != undefined;
      },
      set(v: boolean) {
        this.data.host_key_algorithms = v ? [] : undefined;
      },
    },
    optionVer: {
      get(): boolean {
        return this.data.client_version != undefined;
      },
      set(v: boolean) {
        this.data.client_version = v ? "SSH-2.0-OpenSSH_7.4p1" : undefined;
      },
    },
    host_key: {
      get(): string {
        return this.$props.data.host_key ? this.$props.data.host_key.join("\n") : "";
      },
      set(v: string) {
        this.$props.data.host_key = v.split("\n");
      },
    },
    algorithms: {
      get() {
        return this.$props.data.host_key_algorithms ? this.$props.data.host_key_algorithms.join(",") : "";
      },
      set(v: string) {
        this.$props.data.host_key_algorithms = v.length > 0 ? v.split(",") : undefined;
      },
    },
  },
};
