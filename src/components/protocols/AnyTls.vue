<template>
  <v-card>
    ...
    <v-row v-if="direction == 'in'">
      <v-col cols="12" sm="8">
        <v-textarea label="Padding scheme" auto-grow hide-details v-model="padding_scheme"> </v-textarea>
      </v-col>
    </v-row>
    <v-row v-else>
      ...
      <v-col cols="12" sm="6" md="4">
        <v-text-field :label="$t('types.anytls.minIdle')" type="number" min="0" hide-details v-model.number="minIdle"> </v-text-field>
      </v-col>
    </v-row>
  </v-card>
</template>
...
export default {
  ...
  computed: {
    ...
    idleInterval: {
      get() {
        return this.data.idle_session_check_interval?.length > 0 ? parseInt(this.data.idle_session_check_interval.replace("s", "")) : 30;
      },
      set(v: number) {
    ...
    idleTimeout: {
      get() {
        return this.data.idle_session_timeout?.length > 0 ? parseInt(this.data.idle_session_timeout.replace("s", "")) : 30;
      },
      set(v: number) {
    ...
    minIdle: {
      get() {
        return this.data.min_idle_session != undefined ? this.data.min_idle_session : 0;
      },
      set(v: number) {
