  ...
  <v-card subtitle="ShadowTls">
    <v-row>
      <v-col cols="12" sm="6" md="4">
        <v-select hide-details :items="[1, 2, 3]" :label="$t('version')" :disabled="data.id > 0" v-model="version"> </v-select>
      </v-col>
    ...
    <v-row>
      <v-col cols="12" sm="6" md="4">
        <v-text-field :label="$t('types.shdwTls.hs')" hide-details v-model="Inbound.handshake.server"> </v-text-field>
      </v-col>
      <v-col cols="12" sm="6" md="4">
        <v-text-field :label="$t('out.port')" type="number" min="0" hide-details v-model.number="server_port"> </v-text-field>
      </v-col>
    </v-row>
    <Dial :dial="Inbound.handshake" />
    <v-row v-if="Inbound.handshake_for_server_name != undefined">
      <v-col cols="12" sm="6" md="4">
        <v-text-field :label="$t('types.shdwTls.addHS')" hide-details v-model="handshake_server">
          <template v-slot:append>
            <v-chip color="primary" density="compact" variant="elevated" :disabled="handshake_server == ''" @click="addHandshakeServer()">
    ...
    <v-card v-for="(value, key) in Inbound.handshake_for_server_name" border density="compact" style="margin: 5px" color="background">
      <v-card-title>
        <v-row>
          <v-col
            >{{ key }}
      ...
      <v-row>
        <v-col cols="12" sm="6" md="4">
          <v-text-field :label="$t('types.shdwTls.hs')" hide-details v-model="value.server"> </v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="4">
          <v-text-field :label="$t('out.port')" type="number" min="0" hide-details v-model.number="value.server_port"> </v-text-field>
        </v-col>
      </v-row>
      <Dial :dial="value" />
    </v-card>
...
export default {
  ...
  computed: {
    ...
    server_port: {
      get() {
        return this.Inbound.handshake.server_port ? this.Inbound.handshake.server_port : 443;
      },
      set(newValue: any) {
        this.Inbound.handshake.server_port = newValue.length == 0 || newValue == 0 ? 443 : parseInt(newValue);
      },
    },
  },
  components: { Dial },
