...
export default {
  ...
  computed: {
    allowed_ips: {
      get() {
        return this.$props.data.allowed_ips?.join(",");
      },
      set(v: string) {
        this.$props.data.allowed_ips = v.length > 0 ? v.split(",") : undefined;
      },
    },
    reserved: {
      get() {
    ...
    address: {
      ...
      set(v: string) {
        this.$props.data.address = v.length > 0 ? v : undefined;
      },
    },
    port: {
      get() {
      ...
      set(v: number) {
        this.$props.data.port = v > 0 ? v : undefined;
      },
    },
    keepAlive: {
      get() {
        return this.$props.data.persistent_keepalive_interval ?? 0;
      },
      set(v: number) {
        this.$props.data.persistent_keepalive_interval = v > 0 ? v : undefined;
      },
    },
    privateKey: {
      get() {
    ...
    publicKey: {
      get() {
        return this.$props.data.public_key;
      },
      set(v: string) {
        const indexKeys = this.$props.ext?.keys.findIndex((key: any) => key.public_key == this.$props.data.public_key) ?? -1;
