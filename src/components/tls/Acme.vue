  ...
  <v-card subtitle="ACME" style="background-color: inherit">
    <v-row>
      ...
      <v-col cols="12" md="8" v-if="enabled">
        <v-text-field :label="$t('rule.domain') + ' ' + $t('commaSeparated')" hide-details v-model="domains"> </v-text-field>
      </v-col>
    </v-row>
    <template v-if="enabled">
      <v-row>
        <v-col cols="12" sm="6" md="4" v-if="optionDir">
          <v-text-field :label="$t('tls.acme.dataDir')" hide-details v-model="acme.data_directory"> </v-text-field>
        </v-col>
        ...
        <v-col cols="12" sm="6" md="4" v-if="optionEmail">
          <v-text-field :label="$t('email')" hide-details v-model="acme.email"> </v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row v-if="optionProvider">
        <v-col cols="12" sm="6" md="4">
          <v-select v-model="caProvider" :items="providerList" :label="$t('tls.acme.caProvider')" hide-details></v-select>
        </v-col>
        <v-col cols="12" md="8" v-if="caProvider == ''">
          <v-text-field :label="$t('tls.acme.customCa')" hide-details v-model="acme.provider"> </v-text-field>
        </v-col>
      </v-row>
      <v-row v-if="acme.external_account != undefined">
        <v-col cols="12" sm="6" md="4">
          <v-text-field label="Key ID" hide-details v-model="acme.external_account.key_id"> </v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="4">
          <v-text-field label="MAC Key" hide-details v-model="acme.external_account.mac_key"> </v-text-field>
        </v-col>
      </v-row>
      <v-row v-if="acme.dns01_challenge != undefined">
        ...
        <v-col
          ...
          :key="item"
        >
          <v-text-field :label="$t('tls.acme.dns01Params.' + item)" hide-details v-model="acme.dns01_challenge[item]"> </v-text-field>
        </v-col>
      </v-row>
      <v-card-actions>
        <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    ...
    enabled: {
      get() {
        return this.acme != undefined;
      },
      set(v: boolean) {
        this.$props.tls.acme = v ? { domain: [] } : undefined;
      },
    },
    domains: {
      get() {
        return this.acme?.domain ? this.acme.domain.join(",") : "";
      },
      set(v: string) {
        if (!v.endsWith(",")) {
    ...
    caProvider: {
      get() {
        return this.acme?.provider && ["letsencrypt", "zerossl"].includes(this.acme.provider) ? this.acme?.provider : "";
      },
      set(v: string) {
        this.acme.provider = ["letsencrypt", "zerossl"].includes(v) ? v : "https://";
      },
    },
    optionDir: {
      get(): boolean {
        return this.acme?.data_directory != undefined;
      },
      set(v: boolean) {
        this.acme.data_directory = v ? "" : undefined;
      },
    },
    optionDefault: {
      get(): boolean {
        return this.acme?.default_server_name != undefined;
      },
      set(v: boolean) {
        this.acme.default_server_name = v ? (this.domains.length > 0 ? this.domains[0] : "") : undefined;
      },
    },
    optionEmail: {
      get(): boolean {
        return this.acme?.email != undefined;
      },
      set(v: boolean) {
        this.acme.email = v ? "" : undefined;
      },
    },
    optionChallenge: {
      get(): boolean {
        return this.acme?.disable_http_challenge != undefined || this.acme?.disable_tls_alpn_challenge != undefined;
      },
      set(v: boolean) {
        if (v) {
    ...
    optionPorts: {
      get(): boolean {
        return this.acme?.alternative_http_port != undefined || this.acme?.alternative_tls_port != undefined;
      },
      set(v: boolean) {
        if (v) {
    ...
    optionProvider: {
      get(): boolean {
        return this.acme?.provider != undefined;
      },
      set(v: boolean) {
        this.acme.provider = v ? "letsencrypt" : undefined;
      },
    },
    optionExt: {
      get(): boolean {
        return this.acme?.external_account != undefined;
      },
      set(v: boolean) {
        this.acme.external_account = v ? { key_id: "", mac_key: "" } : undefined;
      },
    },
    optionDns01: {
      get(): boolean {
        return this.acme?.dns01_challenge != undefined;
      },
      set(v: boolean) {
        if (v) {
