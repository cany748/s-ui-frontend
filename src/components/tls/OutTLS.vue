  ...
  <v-card :subtitle="$t('objects.tls')">
    ...
    <template v-if="tls.enabled">
      ...
      <template v-if="optionCert">
        <v-row>
          <v-col cols="auto">
            <v-btn-toggle v-model="usePath" class="rounded-xl" density="compact" variant="outlined" shaped mandatory>
              <v-btn
                @click="
              ...
              <v-btn
                @click="
                  tls.certificate_path = undefined;
                  tls.certificate = '';
                "
                >{{ $t("tls.useText") }}</v-btn
              >
        ...
        <v-row v-if="usePath == 0">
          <v-col cols="12" sm="6">
            <v-text-field :label="$t('tls.certPath')" hide-details v-model="tls.certificate_path"> </v-text-field>
          </v-col>
        </v-row>
        <v-row v-else>
          <v-col cols="12" sm="6">
            <v-textarea :label="$t('tls.cert')" hide-details v-model="tls.certificate"> </v-textarea>
          </v-col>
        </v-row>
      </template>
      <v-row>
        <v-col cols="12" sm="6" md="4" v-if="tls.server_name != undefined">
          <v-text-field label="SNI" hide-details v-model="tls.server_name"> </v-text-field>
        </v-col>
        <v-col cols="12" sm="6" md="4" v-if="tls.alpn">
          <v-select hide-details label="ALPN" multiple :items="alpn" v-model="tls.alpn"> </v-select>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" sm="6" md="4" v-if="tls.min_version">
          <v-select hide-details :label="$t('tls.minVer')" :items="tlsVersions" v-model="tls.min_version"> </v-select>
        </v-col>
        <v-col cols="12" sm="6" md="4" v-if="tls.max_version">
          <v-select hide-details :label="$t('tls.maxVer')" :items="tlsVersions" v-model="tls.max_version"> </v-select>
        </v-col>
      </v-row>
      <v-row v-if="tls.cipher_suites != undefined">
        <v-col cols="12" md="8">
          <v-select hide-details :label="$t('tls.cs')" multiple :items="cipher_suites" v-model="tls.cipher_suites"> </v-select>
        </v-col>
      </v-row>
      <v-row v-if="tls.utls != undefined">
        <v-col cols="12" md="6">
          <v-select hide-details label="Fingerprint" :items="fingerprints" v-model="tls.utls.fingerprint"> </v-select>
        </v-col>
      </v-row>
      <v-row v-if="tls.reality != undefined">
        <v-col cols="12" md="6">
          <v-text-field :label="$t('tls.pubKey')" hide-details v-model="tls.reality.public_key"> </v-text-field>
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field label="Short ID" hide-details v-model="tls.reality.short_id"> </v-text-field>
        </v-col>
      </v-row>
      <template v-if="tls.ech != undefined">
        ...
        <v-row>
          <v-col cols="auto">
            <v-btn-toggle v-model="useEchPath" class="rounded-xl" density="compact" variant="outlined" shaped mandatory>
              <v-btn @click="delete tls.ech?.config">{{ $t("tls.usePath") }}</v-btn>
              <v-btn @click="delete tls.ech?.config_path">{{ $t("tls.useText") }}</v-btn>
            </v-btn-toggle>
          </v-col>
        </v-row>
        <v-row v-if="useEchPath == 0">
          <v-col cols="12" sm="6">
            <v-text-field :label="$t('tls.certPath')" hide-details v-model="tls.ech.config_path"> </v-text-field>
          </v-col>
          <v-col cols="12" sm="6">
            <v-text-field :label="$t('tls.queryServerName')" hide-details v-model="tls.ech.query_server_name" placeholder="ech.example.com">
            </v-text-field>
          </v-col>
        </v-row>
        <v-row v-else>
          <v-col cols="12" sm="6">
            <v-textarea :label="$t('tls.cert')" hide-details v-model="echConfigText"> </v-textarea>
          </v-col>
          <v-col cols="12" sm="6">
            <v-text-field :label="$t('tls.queryServerName')" hide-details v-model="tls.ech.query_server_name" placeholder="ech.example.com">
            </v-text-field>
...
export default {
  ...
  computed: {
    ...
    tlsEnable: {
      get() {
        return Object.hasOwn(this.tls, "enabled") ? this.tls.enabled : false;
      },
      set(newValue: boolean) {
        this.$props.outbound.tls = newValue ? { enabled: true } : { enabled: false };
      },
    },
    disable_sni: {
      get() {
        return this.tls.disable_sni ?? false;
      },
      set(newValue: boolean) {
        this.$props.outbound.tls.disable_sni = newValue ? true : undefined;
      },
    },
    insecure: {
      get() {
        return this.tls.insecure ?? false;
      },
      set(newValue: boolean) {
        this.$props.outbound.tls.insecure = newValue ? true : undefined;
      },
    },
    tlsOptional(): boolean {
    ...
    echConfigText: {
      get(): string {
        return this.tls.ech?.config ? this.tls.ech.config.join("\n") : "";
      },
      set(newValue: string) {
        if (this.tls.ech) this.tls.ech.config = newValue.split("\n");
      },
    },
    optionCert: {
      get(): boolean {
        return this.tls.certificate != undefined || this.tls.certificate_path != undefined;
      },
      set(v: boolean) {
        this.usePath = 0;
        if (v) {
    ...
    optionSNI: {
      get(): boolean {
        return this.tls.server_name != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.server_name = v ? "" : undefined;
      },
    },
    optionALPN: {
      get(): boolean {
        return this.tls.alpn != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.alpn = v ? defaultOutTls.alpn : undefined;
      },
    },
    optionMinV: {
      get(): boolean {
        return this.tls.min_version != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.min_version = v ? defaultOutTls.min_version : undefined;
      },
    },
    optionMaxV: {
      get(): boolean {
        return this.tls.max_version != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.max_version = v ? defaultOutTls.max_version : undefined;
      },
    },
    optionCS: {
      get(): boolean {
        return this.tls.cipher_suites != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.cipher_suites = v ? defaultOutTls.cipher_suites : undefined;
      },
    },
    optionFP: {
      get(): boolean {
        return this.tls.utls != undefined;
      },
      set(v: boolean) {
    ...
    optionReality: {
      get(): boolean {
        return this.tls.reality != undefined;
      },
      set(v: boolean) {
    ...
    optionEch: {
      get(): boolean {
        return this.tls.ech != undefined;
      },
      set(v: boolean) {
        this.$props.outbound.tls.ech = v ? defaultOutTls.ech : undefined;
      },
    },
    optionFragment: {
      get(): boolean {
        return this.tls.fragment != undefined;
      },
      set(v: boolean) {
        if (v) {
    ...
    fragmentFallbackDelay: {
      get(): number {
        return parseInt(this.tls.fragment_fallback_delay?.replace("ms", "") ?? "500") ?? 500;
      },
      set(v: number) {
        this.$props.outbound.tls.fragment_fallback_delay = v > 0 ? `${v}ms` : undefined;
      },
    },
  },
};
