  ...
  <v-card>
    <v-row>
      <v-col cols="12" sm="6" md="3">
        <v-select v-model="ruleToDirect" :items="geoList" :label="$t('setting.toDirect')" multiple chips hide-details></v-select>
      </v-col>
      <v-col cols="12" sm="6" md="3">
        <v-select v-model="ruleToBlock" :items="geoList" :label="$t('setting.toBlock')" multiple chips hide-details></v-select>
      </v-col>
    </v-row>
    <v-row v-if="enableLog">
      <v-col cols="12" sm="6" md="3" lg="2">
        <v-select hide-details :label="$t('basic.log.level')" :items="levels" v-model="subJsonExt.log.level"> </v-select>
      </v-col>
      <v-col cols="12" sm="6" md="3" lg="2">
    ...
    <v-row v-if="enableDns">
      <v-col cols="12" sm="6" md="3" lg="2">
        <v-select hide-details :label="$t('dns.final')" :items="dnsTags" v-model="subJsonExt.dns.final"> </v-select>
      </v-col>
      <v-col cols="12" sm="6" md="3" lg="2">
    ...
    <v-row v-if="enableDns">
      ...
      <v-col cols="12" sm="6" md="3">
        <v-select v-model="dnsToDirect" :items="geositeList" :label="$t('setting.toDirectDns')" multiple chips hide-details></v-select>
      </v-col>
    </v-row>
    <template v-if="enableInb">
      <v-row>
        ...
        <v-col cols="12" sm="6" md="3" lg="2">
          <v-text-field type="number" v-model.number="inbounds[0].mtu" hide-details label="MTU"></v-text-field>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12" sm="6" md="3">
        ...
        <v-col cols="12" sm="6" md="3" lg="2">
          <v-switch v-model="platformProxy" hide-details color="primary" label="Platform HTTP proxy"></v-switch>
        </v-col>
      </v-row>
    </template>
    <v-card-actions>
...
export default {
  ...
  computed: {
    enableLog: {
      ...
      set(v: boolean) {
        v ? (this.subJsonExt.log = this.defaultLog) : delete this.subJsonExt.log;
      },
    },
    enableDns: {
      get(): boolean {
    ...
    enableInb: {
      ...
      set(v: boolean) {
        v ? (this.subJsonExt.inbounds = this.defaultInb.slice()) : delete this.subJsonExt.inbounds;
      },
    },
    enableExp: {
      get(): boolean {
      ...
      set(v: boolean) {
        v ? (this.subJsonExt.experimental = this.defaultExp) : delete this.subJsonExt.experimental;
      },
    },
    dns(): any {
      return this.subJsonExt?.dns ?? undefined;
    ...
    proxyDns: {
      get(): any {
        return this.dns?.servers?.findLast((d: any) => d.tag == "proxy-dns") ?? {};
      },
      set(v: any) {
    ...
    directDns: {
      get(): any {
        return this.dns?.servers?.findLast((d: any) => d.tag == "direct-dns") ?? {};
      },
      set(v: any) {
    ...
    dnsTags() {
      return this.dns?.servers?.map((d: any) => d.tag) ?? [];
    },
    final: {
      get(): string {
        return this.dns.final ?? "";
      },
      set(v: string) {
        this.dns.final = v.length > 0 ? v : undefined;
      },
    },
    dnsToDirect: {
      get(): string[] {
    ...
    platformProxy: {
      get(): boolean {
        return this.inbounds[0]?.platform != undefined;
      },
      set(v: boolean) {
        this.subJsonExt.inbounds[0].platform = v ? this.defaultInb[0].platform : undefined;
      },
    },
    rules(): any {
      return this.subJsonExt?.rules ?? undefined;
    },
    ruleToDirect: {
      get(): string[] {
  ...
  methods: {
    ...
    updateRuleSets() {
      let tags = <string[]>[];
      if (this.dns?.rules?.length > 0)
        this.dns.rules.forEach((r: any) => {
          if (r.rule_set) tags.push(...r.rule_set);
        });
