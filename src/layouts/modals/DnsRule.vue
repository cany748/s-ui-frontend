      ...
      <v-card-text style="padding: 0 16px">
        <v-row>
          ...
          <v-col cols="auto" v-if="logical" justify="center" align="center">
            <v-btn color="primary" @click="ruleData.rules.push(<dnsRule>{})" hide-details>{{
        ...
        >
          ...
          <v-card-text style="padding: 0">
            <RuleOptions :rule="r" :clients="clients" :inTags="inTags" :ruleSets="ruleSets" />
          </v-card-text>
        </v-card>
        <RuleOptions v-else :rule="ruleData.rules[0]" :clients="clients" :inTags="inTags" :ruleSets="ruleSets" />
        <v-row>
          <v-col cols="12" sm="6" md="4">
            <v-select v-model="ruleData.action" :items="actions" :label="$t('dns.rule.action.title')" hide-details></v-select>
          </v-col>
          <v-col cols="12" sm="6" md="4" v-if="logical">
            <v-combobox v-model="ruleData.mode" :items="['and', 'or']" :label="$t('rule.mode')" hide-details></v-combobox>
          </v-col>
          <v-col cols="12" sm="6" md="4">
        ...
        <v-card :subtitle="$t('dns.rule.action.route')" v-if="['route', 'route-options'].includes(ruleData.action)">
          <v-row v-if="ruleData.action == 'route'">
            <v-col cols="12" sm="6" md="4">
              <v-select v-model="ruleData.server" :items="serverTags" :label="$t('dns.server')" hide-details></v-select>
            </v-col>
            <v-col cols="12" sm="6" md="4">
              <v-select
...
export default {
  ...
  computed: {
    ...
    answer: {
      get() {
        return this.ruleData.answer?.length > 0 ? this.ruleData.answer.join(",") : "";
      },
      set(v: string) {
        this.ruleData.answer = v.length > 0 ? v.split(",") : undefined;
      },
    },
    ns: {
      get() {
        return this.ruleData.ns?.length > 0 ? this.ruleData.ns.join(",") : "";
      },
      set(v: string) {
        this.ruleData.ns = v.length > 0 ? v.split(",") : undefined;
      },
    },
    extra: {
      get() {
        return this.ruleData.extra?.length > 0 ? this.ruleData.extra.join(",") : "";
      },
      set(v: string) {
        this.ruleData.extra = v.length > 0 ? v.split(",") : undefined;
      },
    },
  },
  watch: {
