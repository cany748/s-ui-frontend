<template>
  <v-dialog transition="dialog-bottom-transition" width="400">
    <v-card class="rounded-lg" id="qrcode-modal" :loading="loading">
      ...
      <v-divider></v-divider>
      <v-skeleton-loader class="mx-auto border" width="80%" type="text, image, divider, text, image" v-if="loading"></v-skeleton-loader>
      <v-card-text style="overflow-y: auto; padding: 0" :hidden="loading">
        ...
        <v-window v-model="tab" style="margin-top: 10px">
          <v-window-item value="sub">
            ...
            <v-row>
              <v-col style="text-align: center">
                <v-chip>{{ $t("setting.jsonSub") }}</v-chip
                ><br />
                <QrcodeVue
            ...
            <v-row>
              <v-col style="text-align: center">
                <v-chip>{{ $t("setting.clashSub") }}</v-chip
                ><br />
                <QrcodeVue
            ...
            <v-row>
              <v-col style="text-align: center">
                ...
                <QrcodeVue :value="singbox" :size="size" :margin="1" style="border-radius: 0.8rem; cursor: not-allowed" />
              </v-col>
            </v-row>
          </v-window-item>
