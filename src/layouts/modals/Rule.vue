      ...
      <v-card-text style="padding: 0 16px">
        <v-row>
          ...
          <v-col cols="auto" v-if="logical" justify="center" align="center">
            <v-btn color="primary" @click="ruleData.rules.push(<rule>{})" hide-details>{{
        ...
        >
          ...
          <v-card-text style="padding: 0">
            <RuleOptions :rule="r" :clients="clients" :inTags="inTags" :outTags="outTags" :rsTags="rsTags" />
          </v-card-text>
        </v-card>
        <RuleOptions v-else :rule="ruleData.rules[0]" :clients="clients" :inTags="inTags" :outTags="outTags" :rsTags="rsTags" />
        <v-row>
          <v-col cols="12" sm="6" md="4">
            <v-select v-model="ruleData.action" :items="actions" :label="$t('admin.action')" hide-details></v-select>
          </v-col>
        ...
        <v-card subtitle="Route" v-if="ruleData.action == 'route'">
          <v-row>
            <v-col cols="12" sm="6" md="4">
              <v-select v-model="ruleData.outbound" :items="outTags" :label="$t('objects.outbound')" hide-details></v-select>
            </v-col>
          </v-row>
        </v-card>
        ...
        <v-card subtitle="Sniff" v-if="ruleData.action == 'sniff'">
          <v-row>
            <v-col cols="12" sm="6" md="4">
              <v-select v-model="ruleData.sniffer" :items="sniffers" :label="$t('rule.sniffer')" multiple chips hide-details> </v-select>
            </v-col>
            <v-col cols="12" sm="6" md="4">
