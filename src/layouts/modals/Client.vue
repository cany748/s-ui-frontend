            ...
            <v-window-item value="t1">
              ...
              <v-row v-if="id > 0">
                <v-col cols="12" sm="6" md="4" class="d-flex flex-column">
                  ...
                  <v-progress-linear v-model="percent" :color="percentColor" v-if="client.volume > 0" bottom> </v-progress-linear>
                </v-col>
                <v-col cols="12" sm="6" md="4">
            ...
            <v-window-item value="t2">
              ...
              <v-row v-for="key in Object.keys(clientConfig)">
                ...
                <v-col>
                  ...
                  <v-text-field v-if="clientConfig[key].uuid != undefined" label="UUID" v-model="clientConfig[key].uuid" hide-details>
                  </v-text-field>
                  <v-text-field v-if="key == 'vless'" label="Flow" v-model="clientConfig[key].flow" hide-details> </v-text-field>
                  <v-text-field v-if="key == 'hysteria'" label="Auth" v-model="clientConfig[key].auth_str" hide-details> </v-text-field>
                </v-col>
              </v-row>
            </v-window-item>
              ...
              <v-row>
                <v-col>
                  <v-btn color="primary" @click="extLinks.push({ type: 'external', uri: '' })"
                    >{{ $t("actions.add") }} {{ $t("client.external") }}</v-btn
                  >
                </v-col>
              </v-row>
              ...
              <v-row>
                <v-col>
                  <v-btn color="primary" @click="subLinks.push({ type: 'sub', uri: '' })"
                    >{{ $t("actions.add") }} {{ $t("client.sub") }}</v-btn
                  >
                </v-col>
              </v-row>
...
export default {
  ...
  methods: {
    ...
    async saveChanges() {
      ...
      this.client.links = [...this.extLinks.filter((l) => l.uri != ""), ...this.subLinks.filter((l) => l.uri != "")];
  ...
  computed: {
    clientInbounds: {
      get() {
        return this.client.inbounds.length > 0 ? this.client.inbounds.sort() : [];
      },
      set(v: number[]) {
        this.client.inbounds = v.length == 0 ? [] : v.sort();
      },
    },
    expDate: {
      get() {
    ...
    delayStart: {
      get() {
        return this.client.delayStart ?? false;
      },
      set(v: boolean) {
    ...
    autoReset: {
      get() {
        return this.client.autoReset ?? false;
      },
      set(v: boolean) {
    ...
    resetDays: {
      get() {
        return this.client.resetDays ?? 1;
      },
      set(v: number | null) {
        if (!v) v = 1;
    ...
    totalUp(): string {
      return HumanReadable.sizeFormat((this.client.totalUp ?? 0) + this.client.up);
    },
    totalDown(): string {
      return HumanReadable.sizeFormat((this.client.totalDown ?? 0) + this.client.down);
    },
