        ...
        <v-card class="rounded-lg">
          ...
          <template v-if="tlsType == 0">
            <v-row>
              <v-col>
                <v-btn-toggle v-model="usePath" class="rounded-xl" density="compact" variant="outlined" shaped mandatory>
                  <v-btn
                    @click="
                      inTls.key = undefined;
                      inTls.certificate = undefined;
                    "
                    >{{ $t("tls.usePath") }}</v-btn
                  >
                  <v-btn
                    @click="
                      inTls.key_path = undefined;
                      inTls.certificate_path = undefined;
                    "
                    >{{ $t("tls.useText") }}</v-btn
                  >
                </v-btn-toggle>
              ...
              <v-spacer></v-spacer>
              <v-col cols="auto">
                <v-btn variant="tonal" density="compact" icon="mdi-key-star" @click="genSelfSigned" :loading="loading">
                  <v-icon />
          ...
          <template v-if="outTls.reality && inTls.reality">
            <v-row>
              ...
              <v-spacer></v-spacer>
              <v-col cols="auto">
                <v-btn variant="tonal" density="compact" icon="mdi-key-star" @click="genRealityKey" :loading="loading">
                  <v-icon />
                  <v-tooltip activator="parent" location="top">
                    {{ $t("actions.generate") }}
                  </v-tooltip>
            ...
            <v-row>
              ...
              <v-col cols="12" sm="6" md="4" v-if="optionTime">
                <v-text-field label="Max Time Diference" type="number" min="1" :suffix="$t('date.m')" hide-details v-model="max_time">
                </v-text-field>
              </v-col>
            </v-row>
          </template>
...
export default {
  ...
  computed: {
    ...
    certText: {
      get(): string {
        return this.inTls.certificate ? this.inTls.certificate.join("\n") : "";
      },
      set(v: string) {
        this.inTls.certificate = v.split("\n");
      },
    },
    keyText: {
      get(): string {
        return this.inTls.key ? this.inTls.key.join("\n") : "";
      },
      set(v: string) {
        this.inTls.key = v.split("\n");
      },
    },
    disableSni: {
      get() {
        return this.outTls.disable_sni ?? false;
      },
      set(v: boolean) {
        this.tls.client.disable_sni = v ? true : undefined;
      },
    },
    insecure: {
      get() {
        return this.outTls.insecure ?? false;
      },
      set(v: boolean) {
        this.tls.client.insecure = v ? true : undefined;
      },
    },
    server_port: {
      get() {
        return this.inTls.reality?.handshake?.server_port ? this.inTls.reality.handshake.server_port : 443;
      },
      set(v: any) {
    ...
    short_id: {
      get() {
        return this.inTls.reality?.short_id ? this.inTls.reality.short_id.join(",") : undefined;
      },
      set(v: string) {
        if (this.inTls.reality) {
    ...
    max_time: {
      get() {
        return this.inTls?.reality?.max_time_difference ? this.inTls.reality.max_time_difference.replace("m", "") : 1;
      },
      set(v: number) {
        if (this.inTls.reality) {
    ...
    optionSNI: {
      get(): boolean {
        return this.inTls.server_name != undefined;
      },
      set(v: boolean) {
        this.inTls.server_name = v ? "" : undefined;
      },
    },
    optionALPN: {
      get(): boolean {
        return this.inTls.alpn != undefined;
      },
      set(v: boolean) {
        this.inTls.alpn = v ? defaultInTls.alpn : undefined;
      },
    },
    optionMinV: {
      get(): boolean {
        return this.inTls.min_version != undefined;
      },
      set(v: boolean) {
        this.inTls.min_version = v ? defaultInTls.min_version : undefined;
      },
    },
    optionMaxV: {
      get(): boolean {
        return this.inTls.max_version != undefined;
      },
      set(v: boolean) {
        this.inTls.max_version = v ? defaultInTls.max_version : undefined;
      },
    },
    optionCS: {
      get(): boolean {
        return this.inTls.cipher_suites != undefined;
      },
      set(v: boolean) {
        this.inTls.cipher_suites = v ? defaultInTls.cipher_suites : undefined;
      },
    },
    optionFP: {
      get(): boolean {
        return this.outTls.utls != undefined;
      },
      set(v: boolean) {
        this.outTls.utls = v ? defaultOutTls.utls : undefined;
      },
    },
    optionStore: {
      get(): boolean {
        return this.inTls.store != undefined;
      },
      set(v: boolean) {
        this.inTls.store = v ? "mozilla" : undefined;
      },
    },
    optionKtls: {
      get(): boolean {
        return this.inTls.kernel_tx != undefined || this.inTls.kernel_rx != undefined;
      },
      set(v: boolean) {
        if (v) {
    ...
    optionEch: {
      get(): boolean {
        return this.outTls.ech != undefined;
      },
      set(v: boolean) {
        this.outTls.ech = v ? defaultOutTls.ech : undefined;
      },
    },
    optionTime: {
      get(): boolean {
        return this.inTls?.reality?.max_time_difference != undefined;
      },
      set(v: boolean) {
        if (this.inTls.reality) this.inTls.reality.max_time_difference = v ? "1m" : undefined;
      },
    },
  },
  watch: {
