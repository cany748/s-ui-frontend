          ...
          <v-window v-model="tab">
            <v-window-item value="t1">
              ...
              <v-row v-if="!NoServer.includes(outbound.type)">
                <v-col cols="12" sm="6" md="4">
                  <v-text-field v-model="outbound.server" :label="$t('out.addr')" hide-details> </v-text-field>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                  <v-text-field v-model.number="outbound.server_port" :label="$t('out.port')" type="number" min="0" hide-details>
                  </v-text-field>
                </v-col>
              </v-row>
...
export default {
  ...
  methods: {
    ...
    changeType() {
      ...
      // Use previous data
      const prevConfig = { id: this.outbound.id, tag, listen: this.outbound.listen, listen_port: this.outbound.listen_port };
