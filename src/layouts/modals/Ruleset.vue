        ...
        <v-row>
          <v-col cols="12" sm="6" md="4">
            <v-select
              ...
              :label="$t('type')"
              :items="[
                { title: $t('ruleset.local'), value: 'local' },
                { title: $t('ruleset.remote'), value: 'remote' },
              ]"
          ...
          <v-col cols="12" sm="6" md="4">
            <v-select hide-details :label="$t('ruleset.format')" :items="['source', 'binary']" v-model="rule_set.format"> </v-select>
          </v-col>
        </v-row>
...
export default {
  ...
  computed: {
    update_intervals: {
      get() {
        return this.rule_set.update_interval != undefined ? parseInt(this.rule_set.update_interval.replace("d", "")) : 0;
      },
      set(v: number) {
        this.rule_set.update_interval = v > 0 ? v + "d" : undefined;
      },
    },
  },
  watch: {
