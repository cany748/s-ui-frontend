<template>
  <v-dialog transition="dialog-bottom-transition" width="800" :model-value="visible">
    <v-card class="rounded-lg">
      <v-card-title> {{ $t("actions.addbulk") }} {{ $t("objects.outbound") }} </v-card-title>
      <v-divider></v-divider>
      <v-card-text style="padding: 0 16px; overflow-y: scroll">
        ...
        >
          ...
          <template v-slot:[`item.server`]="{ item }"> {{ item.server }}{{ item.server_port ? ":" + item.server_port : "" }} </template>
        </v-data-table>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" variant="outlined" @click="closeModal">{{ $t("actions.close") }}</v-btn>
        <v-btn color="primary" variant="tonal" :loading="loading" :disabled="outbounds.length == 0" @click="saveChanges">{{
          $t("actions.save")
        }}</v-btn>
      </v-card-actions>
    </v-card>
