    ...
    <v-col cols="12" sm="4" md="3" lg="2" v-for="(item, index) in <any[]>rulesets" :key="item.tag">
      <v-card rounded="xl" elevation="5" min-width="200" :title="item.tag">
        <v-card-subtitle style="margin-top: -15px">
          <v-row
            ><v-col>{{ $t("ruleset." + item.type) }}</v-col></v-row
          >
        </v-card-subtitle>
        <v-card-text>
          <v-row
            ><v-col>{{ $t("ruleset.format") }}</v-col
            ><v-col>{{ item.format }}</v-col></v-row
          >
          <v-row
            ><v-col>{{ $t("objects.outbound") }}</v-col
            ><v-col>{{ item.download_detour ?? "-" }}</v-col></v-row
          >
          <v-row
            ><v-col>{{ $t("actions.update") }}</v-col
            ><v-col>{{ item.update_interval ?? "-" }}</v-col></v-row
          >
        </v-card-text>
        <v-divider></v-divider>
  ...
  <v-row>
    ...
    >
      <v-card rounded="xl" elevation="5" min-width="200" :title="index + 1">
        <v-card-subtitle style="margin-top: -15px">
          <v-row
            ><v-col>{{ item.type != undefined ? $t("rule.logical") + " (" + item.mode + ")" : $t("rule.simple") }}</v-col></v-row
          >
        </v-card-subtitle>
        <v-card-text>
          <v-row
            ><v-col>{{ $t("admin.action") }}</v-col
            ><v-col>{{ item.action }}</v-col></v-row
          >
          <v-row
            ><v-col>{{ $t("objects.outbound") }}</v-col
            ><v-col>{{ item.outbound ?? "-" }}</v-col></v-row
          >
          <v-row
            ><v-col>{{ $t("pages.rules") }}</v-col
            ><v-col>{{ item.rules ? item.rules.length : Object.keys(item).filter((r) => !actionKeys.includes(r)).length }}</v-col></v-row
          >
          <v-row
            ><v-col>{{ $t("rule.invert") }}</v-col
            ><v-col>{{ $t((item.invert ?? false) ? "yes" : "no") }}</v-col></v-row
          >
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions style="padding: 0">
...
const outboundTags = computed((): string[] => [...Data().outbounds?.map((o: any) => o.tag), ...Data().endpoints?.map((e: any) => e.tag)]);
