<template>
  <ClientModal
    ...
    @close="closeModal"
  />
  <ClientAddBulk v-model="addBulkModal" :visible="addBulkModal" :groups="groups" :inboundTags="inboundTags" @close="closeAddBulk" />
  <ClientEditBulk v-model="editBulkModal" :visible="editBulkModal" :inboundTags="inboundTags" :clients="clients" @close="closeEditBulk" />
      ...
      <v-menu v-model="filterMenu" :close-on-content-click="false" location="bottom center">
        ...
        <v-card>
          <v-container>
            <v-row>
              <v-col>
                <v-select variant="underlined" density="compact" :label="$t('type')" :items="filterItems" v-model="filterSettings.state">
                </v-select>
              </v-col>
            </v-row>
            <v-row>
            ...
            <v-row>
              <v-col>
                <v-text-field variant="underlined" density="compact" :label="$t('client.name')" v-model="filterSettings.text">
                </v-text-field>
              </v-col>
            </v-row>
          </v-container>
    ...
    <v-col cols="12">
      ...
      >
        ...
        <template v-slot:item.volume="{ item }">
          ...
          >
            <v-chip size="small" :color="item.volume == 0 ? 'success' : item.volume <= item.up + item.down ? 'error' : ''" label>{{
              HumanReadable.sizeFormat(item.up + item.down) +
              " / " +
              (item.volume == 0 ? $t("unlimited") : HumanReadable.sizeFormat(item.volume))
            }}</v-chip>
          </div>
          <v-progress-linear :model-value="percent(item)" :color="percentColor(item)" v-if="item.volume > 0" bottom> </v-progress-linear>
        </template>
        <template v-slot:item.expiry="{ item }">
          <div class="text-start">
            ...
            />
            <v-chip size="small" :color="item.expiry == 0 ? 'success' : item.expiry <= Date.now() / 1000 ? 'error' : ''" label>{{
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-menu v-model="delOverlay[clients.findIndex((c) => c.id == item.id)]" :close-on-content-click="false" location="top center">
            <template v-slot:activator="{ props }">
            ...
            <v-card :title="$t('actions.del')" rounded="lg">
              ...
              <v-card-actions>
                ...
                <v-btn color="success" variant="outlined" @click="delOverlay[clients.findIndex((c) => c.id == item.id)] = false">{{
                  $t("no")
                }}</v-btn>
              </v-card-actions>
            </v-card>
          </v-menu>
          <v-icon class="me-2" @click="showQrCode(item.id)"> mdi-qrcode </v-icon>
