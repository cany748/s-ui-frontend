<template>
  ...
  <QrCode v-model="qrcode.visible" :visible="qrcode.visible" :data="qrcode.data" @close="closeQrCode" />
  <v-row>
    ...
    <v-col cols="12" sm="4" md="3" lg="2" v-for="(item, index) in <any[]>endpoints" :key="item.tag">
      <v-card rounded="xl" elevation="5" min-width="200" :title="item.tag">
        ...
        <v-card-actions style="padding: 0">
          ...
          <v-icon class="me-2" v-if="item.type == 'wireguard' && item.peers?.length > 0" @click="showQrCode(item.id)"> mdi-qrcode </v-icon>
