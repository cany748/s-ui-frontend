    ...
    <v-col cols="12" sm="4" md="3" lg="2" v-for="(item, index) in <any[]>dns.servers" :key="item.id">
      <v-card rounded="xl" elevation="5" min-width="200" :title="item.tag">
        ...
        <v-card-actions style="padding: 0">
          ...
          <v-overlay v-model="delDnsOverlay[index]" contained class="align-center justify-center">
...
const tsTags = computed((): string[] => {
  return Data()
    .endpoints?.filter((e: any) => e.type == "tailscale")
    .map((e: any) => e.tag);
});

const rslvdTags = computed((): string[] => {
  return Data()
    .services?.filter((e: any) => e.type == "resolved")
    .map((e: any) => e.tag);
});
...
const finalDns = computed({
  get() {
    return dns.value?.final ?? "";
  },
  set(v: string) {
    dns.value.final = v.length > 0 ? v : undefined;
  },
});
